...
본 시스템은 Netty의 멀티스레드 이벤트 루프가 패킷을 수신하며, 메모리상에 독립된 2개의 CMS(Count-Min Sketch) 매트릭스를 두고 1초 주기로 역할을 교대(Swap)합니다.

```text
[ 짝수 초 (Even Sec) ]                 [ 홀수 초 (Odd Sec) ]
┌───────────────────┐                 ┌───────────────────┐
│   Matrix A (활성)  │ ── 1초 경과 ──> │   Matrix A (대기)  │ ──> 제로 가비지 초기화
│  → 멀티스레드 카운트│                 │   (memset / 0 리셋)│
└───────────────────┘                 └───────────────────┘
▲                                     ▲
┌───────────────────┐                 ┌───────────────────┐
│   Matrix B (대기)  │ ── 1초 경과 ──> │   Matrix B (활성)  │
│   (초기화 완료 상태)│                 │  → 멀티스레드 카운트│
└───────────────────┘                 └───────────────────┘
```
...
2. O(1) 초고속 만료 (Time-slot Swapping): 개별 IP마다 타이머를 돌리지 않고, 1초 주기의 백그라운드 타이머가 AtomicReference를 이용해 활성 매트릭스 포인터를 순간적으로 스왑합니다.
